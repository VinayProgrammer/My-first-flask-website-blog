dfrom flask import Flask, render_template, request, session, redirect
...
def main():
    ...
    app = Flask(__name__)
    app.secret_key = "super-secret-key-2007"
    ...
    #     posts = Posts.query.filter_by().all()
        # )last = math.floor(len(posts)/int(params["no-of-post"]))
        # # [0:params["no-of-post"]]
        # # posts = posts[]
        # page = (request.args.get('page'))
        # if not str(page).isnumeric():
        #     page = 1
        # posts = posts[page*int(params["no-of-post"]): page*int(params["no-of-post"]) + page*int(params["no-of-post"])]
        # if page == 1:
        #     prev = "#"
    ...
    def edit(sno):
        if 'user' in session and session['user'] == params['admin_user']:
            if request.method == 'POST':
                ...
                if sno == '0':
                    post = Posts(title=box_title, tagline=tline, content=content, slug=slug, img_file=img_file, date=date)
                    db.session.add(post)
                    db.session.commit()
